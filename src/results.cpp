...
void Results::prepareWidgets()
{
    ...
    // Fill the angles
    {
        ...
        size_t nJoints(GetMainWindow().GetJointAngle().at(0).GetNumberOfJoints());
        for (size_t j = 0; j < nJoints; ++j){
            QtCharts::QSplineSeries *serie = new QtCharts::QSplineSeries();
            serie->setName(GetMainWindow().GetJointAngle().at(0).GetJoint(j).GetName().c_str());
...
void Results::resizeEvent(QResizeEvent *event)
{
    ...
    if (legend->size().width() != 0.0 && legend->size().height() != 0.0)
        legend->setGeometry(cv->geometry().width()/2 - legend->size().width() / 2,
                            cv->geometry().height() - legend->size().height() * 1.5,
                            legend->size().width(),
                            legend->size().height());
}

void Results::resizeWidgetSubplot(QWidget * widget, int row, int numberRow, int col, int numberCol, double aspectRatio){
    double rectHeight(ui->NextWidget->geometry().top() * 1.0/static_cast<double>(numberRow) - _margin);
    double rectWidth(size().width() * 1.0/static_cast<double>(numberCol) - _margin);
    QRect newSize;
...
void Results::on_NextWidget_clicked()
{
    if (ui->modelPositionWidget->count() != 0)
        ui->modelPositionWidget->setCurrentIndex((ui->modelPositionWidget->currentIndex() + 1) % ui->modelPositionWidget->count());
}
